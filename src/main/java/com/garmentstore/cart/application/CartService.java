...
public class CartService {
        ...
        for (CartItemRequest item : req.items()) {
            try {
                ...
                results.add(new CartValidationResponse.ItemValidation(
                        variant.getId(), variant.getSizeCode(),
                        product.getName(),
    ...
    private CartItemResponse toResponse(CartItem item) {
        ...
        return new CartItemResponse(
                ...
                p.getGenderTag().name(),
                v.getId(), v.getSizeCode(), v.getSkuCode(),
                v.getStockQuantity(), item.getQuantity(),
                p.getSellingPrice(), p.getMrp(), p.getDiscountPercent()
        );
    }
    ...
    private void validateStock(ProductVariant variant, int requestedQty) {
        if (variant.getStockQuantity() <= 0) {
            throw new BusinessException(
                    "OUT_OF_STOCK",
                    "Size " + variant.getSizeCode() + " is out of stock",
                    HttpStatus.CONFLICT);
        }
        if (requestedQty > variant.getStockQuantity()) {
            throw new BusinessException(
                    "INSUFFICIENT_STOCK",
                    "Only " + variant.getStockQuantity() + " unit(s) available in size " + variant.getSizeCode(),
                    HttpStatus.CONFLICT);
        }
    }
}
